...
# Fix mysqldump permision
sed -i 's/\DEFINER\=`[^`]*`@`[^`]*`//g' $TMP_DIR/$DB_NAME.sql
...
echo czf $DUMPS_DIR/files.tgz -C $STATIC_FILES_DIR .
sudo tar czf $DUMPS_DIR/files.tgz -C $STATIC_FILES_DIR .
